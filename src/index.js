...
import { visitors } from '@nkduy/babel-traverse';
...
function isPathDulcetClass(path) {
  if (path.matchesPattern('Dulcet.Component') ||
    path.matchesPattern('Dulcet.PureComponent')) {
    return true;
  }
      ...
      Program(programPath, state) {
        ...
        if (globalOptions.removeImport) {
          if (globalOptions.mode === 'remove') {
            ...
            throw new Error(
              'dulcet-remove-prop-types: removeImport and mode=remove can not be used at the same time.',
            );
          }
        }
      },
